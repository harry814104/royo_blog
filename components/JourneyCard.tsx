...
export default function JourneyCard({ journey }: { journey: Journey }) {
  ...
  return (
    <Link
      ...
      className="group nordic-card flex overflow-hidden transition-transform hover:-translate-y-0.5"
    >
      {/* 封面 / 色塊 */}
      <div
        className="relative hidden w-40 shrink-0 sm:block"
      ...
      >
        {journey.cover ? (
          ...
          <span className="absolute bottom-3 left-3 text-[11px] font-medium tracking-[0.14em] text-cream/80">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h3 className="text-xl font-medium leading-7 tracking-tight transition-colors group-hover:text-forest">
          {journey.name}
        </h3>
        <p className="mt-1 text-xs text-ink-mute">{formatRange(journey)}</p>
        <p className="mt-2 text-sm leading-7 text-ink-soft">{journey.excerpt}</p>
